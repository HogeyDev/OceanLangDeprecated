...
unsigned int is_datatype(token_T* token) {
	if (token->type != TOKEN_ID) return 0;
	if (strcmp(token->value, "int") == 0) return 1;
	if (strcmp(token->value, "float") == 0) return 1;
	if (strcmp(token->value, "char") == 0) return 1;
	/*if (strcmp(token->value, "short") == 0) return 1;*/
	/*if (strcmp(token->value, "long") == 0) return 1;*/
	/*if (strcmp(token->value, "double") == 0) return 1;*/

	return 0;
}

char* get_string_contents(char* str) {
	char* out_str = calloc(strlen(str)-1, sizeof(char));
	int max_index = strlen(str)-1;
	for (int i = 1; i < max_index; i++) {
		strcat(out_str, (char[]){str[i], 0});
	}
	return out_str;
}
