    ...
    public:
        ...
        std::string compileRootCompound(AST* ast, int ismain=0) {
            ...
            for (auto& child : ast->children) {
                switch (child->type) {
                    case AST_IMPORT: ret += this->compileImport(child, scope) + "\n"; break;
                    case AST_EXTERN: ret += this->compileExtern(child, scope) + "\n"; break;
                    case AST_DECLARATION: ret += this->compileDeclaration(child, scope) + "\n"; break;
                    case AST_FUNCTION_CALL: ret += this->compileFunctionCall(child, scope) + "\n"; break;
                    case AST_VARIABLE_ASSIGNMENT: ret += this->compileVariableAssignment(child, scope) + "\n"; break;
                    default: {
        ...
        std::string compileDeclaration(AST* ast, Scope* scope) {
            ...
            if (ast->declarator->value != "main") ret += "push rbp\nmov rbp, rsp\n";
        ...
        std::string compileFunctionCall(AST* ast, Scope* scope) {
            ...
            for (auto& a : ast->arguments) {
                ...
                if (a->type == AST_VARIABLE) {
                    ...
                    std::string offsetString = "QWORD [rsp+" + std::to_string(offset) + "]\n";
                    pushed = offsetString;
                }
                if (a->declType == "str") {
            ...
            ret += "call " + ast->value + "\nadd rsp, " + std::to_string(ast->arguments.size() * 8);
            return ret;
        }
